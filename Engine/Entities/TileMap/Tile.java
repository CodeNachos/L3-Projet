...
public class Tile extends GameObject {
    ...
    public Tile(TileMap map, int line, int column, Image sprite) {
        super(
            ...
            new Vector2D(
                (double)map.tileDimension.width / (double)sprite.getWidth(null), // Calculate x-scale based on tile width and sprite width
                (double)map.tileDimension.height / (double)sprite.getHeight(null) // Calculate y-scale based on tile height and sprite height
            ),
            sprite
        );
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawImage(sprite, position.getIntX(), position.getIntY(), getSize().width, getSize().height, null); // Draw the tile sprite
    }
}
